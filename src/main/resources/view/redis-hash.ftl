...
<body>
	...
	<div class="info" id="info">
		<div class="ui mini secondary menu">
			...
			<div class="header  item">
				<h4>Type: <span style="user-select: text;">hash</span></h4> 
			</div>
			<div class="header  item">
		...
		<table class="ui compact small single line table red" id="thead">
			<thead>
			  <tr>
				...
				<th class="three wide">Member</th>
				<th class="five wide">Value</th>
	...
	<div class="ui modal mini" id="newbox">
		...
		<form class="ui content form">
			  ...
			  <div class="field">
				<label>Value:</label>
				<div class="ui input ">
					<input id="score" type="text" placeholder="请输入Value">
				</div>
			</div>
		</form>
		<div class="actions">
...
<script>
	...
	function init(){
		...
		$("#next").click(function(){
			var page=$("#page").text();
			if(hashCursor.get(Number(page)+1)=="-1"){
				return;
			}
			pagevalue(Number(page)+1);
		});
	 ...
	 function find(match){
			...
			$.ajax({
					...
					success:function(data){
						if(data.ok){
							hashCursor.clear();
							hashCursor.set(2,data.data.cursor);
							showdata(data.data);
						}else{
							alert(data.message);
						}
		...
		function pagevalue(page){
			...
			data["page"]=1;
			data["accurate"]=accurate==='1'?true:false;
			data["cursor"]=hashCursor.get(page);
			...
			$.ajax({
				...
				success:function(data){
					if(data.ok){
						hashCursor.set(page+1,data.data.cursor);
						showdata(data.data);
					}else{
						alert(data.message);
					}
